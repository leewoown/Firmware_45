...
// Checkin $Date: January 4, 2011   10:02:33 $ 
//###########################################################################
//
...
// functions called by Example_F2806xEqep_freqcal.c.  The frequency calculation
// steps performed by FREQCAL_Calc()at  SYSCLKOUT = 80 MHz are
// described below:
//
...
//										  = QCPRDLAT
//
//		        So: 10 kHz = 8(80MHz/128)/2(t2-t1)
//		             t2-t1 = 8(80MHz/128)/(10kHz*2) = (80MHz/128)/(2*10kHz/8)  - Equation 5
//		                   = 250 QCAPCLK cycles = maximum (t2-t1) = freqScaler_pr
//
// Divide both sides by (t2-t1), and:
//                   1 = 250/(t2-t1) = [(80MHz/128)/(2*10kHz/8)]/(t2-t1)
//				     Because (t2-t1) must be <250 (max).
//				     250/(t2-t1) < 1 for all frequencies less than max
//					 freq_pr = 250/(t2-t1) or [(80MHz/128)/(2*10kHz/8)]/(t2-t1)  - Equation 6
// Now within velocity limits, to get back to original velocity equation, Equation 1,
// multiply Equation 6 by 10 kHz:
//                  freqhz_fr (or velocity) = 10kHz*[80MHz/128)/(2*10kHz/8)]/(t2-t1)
//							                = (80MHz/128)*8/[2(t2-t1)]
